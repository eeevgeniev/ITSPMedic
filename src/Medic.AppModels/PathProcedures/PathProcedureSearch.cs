...
{
    public class PathProcedureSearch : IQueryStringBuilder
    {
        ...
        public PathProcedureOrderEnum Order { get; set; }
        ...
        public Dictionary<string, string> BuildQuery(string prefix)
        {
            ...
            if (UsedDrugCode != default)
            {
                queryString.Add($"{(prefix != default ? $"{prefix}." : default)}{nameof(UsedDrugCode)}", UsedDrugCode.ToString());
            }
        ...
        public override string ToString()
        {
            return $"{nameof(FirstMainDiagCode)}:{FirstMainDiagCode}&{nameof(SecondMainDiagCode)}:{SecondMainDiagCode}" +
                $"&{nameof(UsedDrugCode)}:{UsedDrugCode}&{nameof(Sex)}:{Sex}" +
